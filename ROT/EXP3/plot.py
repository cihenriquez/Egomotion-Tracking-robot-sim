...
def main():
    ...
    for category, columns in categories.items():
        ...
        for col_idx in columns:
            ax.plot(time, data[:, col_idx], label=column_names[col_idx], linewidth=1)
    ...
    variables = {
        "Mass [kg]": (19, 23),
        "Linear Viscous Friction Coefficient "+ r"$[ \frac{N}{m/s} ]$": (20, 24),
        "Inertia Moment [kg$\\cdot$m$^2$]": (21, 25),
        "Rotational Viscous Friction Coefficient "+ r"$[ \frac{N \cdot m}{rads/s} ]$": (22, 26)
    }

    # Plot each variable
    for var_name, (est_idx, actual_idx) in variables.items():
        fig, ax = plt.subplots()
        ax.plot(time, data[:, est_idx], label=f"Estimated value", linestyle='--', linewidth=1)
        ax.plot(time, data[:, actual_idx], label=f"Actual value", linewidth=1)
